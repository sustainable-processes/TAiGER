...
def create_pyomo_model(input_profile):
   ...
   # Define equations
   def _eq1_rule(m, i, n):
      return (m.interface13_F_conv[i,n] == (m.interface12_F_conv[i,n] + 0))
   m.eq1 = Constraint(m.i, m.n, rule=_eq1_rule)

   def _eq2_rule(m, i, n):
      return (m.interface13_V_dot_conv[n] == (m.interface12_V_dot_conv[n] + 0))
   ...
   def _eq5_rule(m, i, p, n):
      return (0 == ((m.interface8_F_conv[i] - m.interface9_F_conv[i,n]) + (m.compartment5_V * m.compartment5_phase1R[i,p,n])))
   ...
   def _eq7_rule(m, i, n):
      return (m.interface12_F_conv[i,n] == (m.interface5_F_conv[i,n] + m.interface9_F_conv[i,n]))
   m.eq7 = Constraint(m.i, m.n, rule=_eq7_rule)

   def _eq8_rule(m, i, n):
      return (m.interface12_V_dot_conv[n] == (m.interface5_V_dot_conv[n] + m.interface9_V_dot_conv[n]))
   m.eq8 = Constraint(m.i, m.n, rule=_eq8_rule)

   def _eq9_rule(m, i, p, n):
      return (0 == ((m.interface13_F_conv[i,n] - m.interface14_F_conv[i,n]) + ((m.compartment9_V * m.compartment9_phase2R[i,p,n]) + (m.compartment9_V * m.compartment9_phase1R[i,p,n]))))
   ...
   def _eq11_rule(m, n):
      return (0.105e5 == (m.compartment2_V + (m.compartment5_V + m.compartment9_V)))
