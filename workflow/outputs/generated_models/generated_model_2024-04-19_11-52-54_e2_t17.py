...
def create_pyomo_model(input_profile):
   ...
   # Begin: Block 2
   # Define parameters
   m.ord_ip = Param(m.i, m.p, initialize={(1, 1): 1, (2, 1): 0, (3, 1): 0})
   m.d_v_z_dz = Param(m.z, m.i, initialize=0)
   m.T = Param(initialize=298)
   m.d_j_i_dz = Param(m.z, m.i, initialize=0)
   m.E_p = Param(m.p, initialize={1: 100})
   m.v_z = Param(initialize=0.001)
   m.k_ref_p = Param(m.p, initialize=input_profile["k_ref_p"], mutable=True) # This parameter is to be estimated.
   m.R = Param(initialize=8.314)
   m.T_ref = Param(initialize=300)
   ...
   m.eq4 = Constraint(m.z, m.p, rule=_eq4_rule)

   def _eq5_rule(m, p):
      return (0 == ((m.k_ref_p[p] * exp(((m.E_p[p] / m.R) * ((1 / m.T_ref) - (1 / m.T))))) - m.k_p[p]))
   m.eq5 = Constraint(m.p, rule=_eq5_rule)
